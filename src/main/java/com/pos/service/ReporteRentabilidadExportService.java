...
import com.itextpdf.layout.element.Cell; // PDF
import com.itextpdf.layout.element.Table; // PDF
...
@Service
public class ReporteRentabilidadExportService {

    private static final DateTimeFormatter FECHA_FORMATO = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm");

    // ----------------- PDF -----------------
    public byte[] generarReporteRentabilidadPDF(ReporteRentabilidadDTO reporte) {
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            ...
            // Totales destacados
            Table tablaTotales = new Table(UnitValue.createPercentArray(new float[]{50, 50}))
            ...
            tablaTotales.addCell(new Cell().add(new Paragraph("Ganancia Neta").setBold()).setTextAlignment(TextAlignment.CENTER));
            tablaTotales.addCell(new Cell().add(new Paragraph(String.format("$%,.2f", reporte.getGananciaNeta())).setBold()).setTextAlignment(TextAlignment.CENTER));

            document.add(tablaTotales);
            document.add(new Paragraph("\n"));

            // Tabla Ventas
            Table tablaVentas = new Table(UnitValue.createPercentArray(new float[]{40, 80, 80, 60, 80, 60, 60}))
                    .useAllAvailableWidth();
            ...
            // Tabla Gastos
            Table tablaGastos = new Table(UnitValue.createPercentArray(new float[]{40, 80, 200, 60}))
                    .useAllAvailableWidth();
    ...
    // ----------------- EXCEL -----------------
    public byte[] generarReporteRentabilidadExcel(ReporteRentabilidadDTO reporte) {
        try (Workbook workbook = new XSSFWorkbook();
            ...
            filaNum = crearFilaTotales(sheet, filaNum, "Ganancia Neta", reporte.getGananciaNeta(), estiloMoneda);
            filaNum++;
            ...
            for (int i = 0; i < columnasVentas.length; i++) {
                headerVentas.createCell(i).setCellValue(columnasVentas[i]);
            }
            ...
            String[] columnasGastos = {"ID", "Fecha", "Descripción", "Valor"};
            for (int i = 0; i < columnasGastos.length; i++) {
                headerGastos.createCell(i).setCellValue(columnasGastos[i]);
            }
